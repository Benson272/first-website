$(document).ready(function(){
	...
	$('#restart').click(function(){
		...
		stopwatchListAll();
		numberOfLap = 0;
		$('.lapContainer p').remove();
	});

	var numberOfLap = 0;
	var lapTime = function(){
		$('<p>').prependTo('.lapContainer').text(numberOfLap + ") "+ stopwatchMinute + ":" + stopwatchTensSecond + stopwatchSecond + "."+stopwatchMs).addClass('lapTime');
		numberOfLap +=1;
	};
	
	var lap = document.getElementById('lap');
	lap.addEventListener('mousedown',lapTime);
